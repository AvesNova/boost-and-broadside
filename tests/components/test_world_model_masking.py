...
def test_world_model_masking_behavior():
    ...
    assert diff > 1e-6, "Masked forward pass should produce different outputs than clean pass"
...
def test_world_model_mask_token_usage():
    ...
    assert diff_masked < 1e-6, f"Masked token embedding should be invariant to input content changes. Diff: {diff_masked}"
    ...
    assert diff_unmasked > 1e-6, "Unmasked token embedding SHOULD change when input changes"
...
def test_first_token_never_masked():
    ...
    _, _, _, mask, _ = model(
        states, actions, mask_ratio=mask_ratio
    )
