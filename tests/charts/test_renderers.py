...
from boost_and_broadside.charts import registered_renderers
from boost_and_broadside.charts.renderer_api import (
    PublicationError,
    RenderInputs,
    get_renderer,
)
from boost_and_broadside.charts.renderers.ar_report import (
    _calc_toroidal_euclidean,
    _clamp_alive_prob,
    _toroidal_center_of_mass,
    _unwrap_1d,
)
from boost_and_broadside.charts.renderers.elo_scale import combine_reference_ladder
from boost_and_broadside.charts.renderers.training import _NEXT_STATE
...
def test_elo_scale_joins_an_optional_reference_ladder(tmp_path) -> None:
    ...
    written = _render("elo-scale-v1", {"scale": scale, "reference": ladder}, tmp_path / "out")
...
def test_the_calibration_diagnostics_render_both_draw_conventions(tmp_path) -> None:
    ...
    written = _render("elo-calibration-diagnostics-v1", {"calibration": artifact}, tmp_path / "out")
...
def test_no_calibration_figure_renders_off_its_own_canvas(tmp_path) -> None:
    ...
    oversized = {name: size for name, size in sizes.items() if max(size) > _MAX_FIGURE_PIXELS}
