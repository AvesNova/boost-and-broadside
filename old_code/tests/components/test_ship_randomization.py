...
def test_random_speed_initialization():
    ...
    assert (speed[alive] >= config.min_speed - 1e-4).all(), (
        f"Speeds should be >= {config.min_speed}"
    )
    assert (speed[alive] <= config.max_speed + 1e-4).all(), (
        f"Speeds should be <= {config.max_speed}"
    )
...
def test_no_random_speed_initialization():
    ...
    config = ShipConfig(random_speed=False, world_size=(1000, 1000), dt=0.015)

    env = TensorEnv(num_envs=10, config=config, device="cpu", max_ships=8)
    ...
    assert (speed > 99.9).all() and (speed < 100.1).all(), (
        "Speeds should be 100.0 with random_speed=False"
    )
