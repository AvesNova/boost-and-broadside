...
def test_mamba_training_pipeline(tmp_path, synthetic_mamba_data):
    ...
    cfg = OmegaConf.create(
        {
            "model": mamba_cfg,
            "environment": {"world_size": [100.0, 100.0], "n_ships": 4},
            "train": {
                "amp": False,
                "seed": 42,
            ...
            "collect": {"output_dir": str(tmp_path / "data")},
            "wandb": {
                "enabled": False,
                "project": "test",
                "entity": "test",
    ...
    cfg.model.scheduler = OmegaConf.create(
        {"type": "warmup_constant", "warmup": {"steps": 1, "start_lr": 1e-5}}
    )
    ...
    with (
        patch(
            "boost_and_broadside.train.data_loader.get_latest_data_path",
            return_value=h5_path,
        ),
