...
def test_pretraining_pipeline(tmp_path):
    ...
    with h5py.File(h5_path, "w") as f:
        f.create_dataset(
            "tokens",
            data=np.random.randn(num_samples, max_ships, token_dim).astype(np.float32),
        )
        f.create_dataset(
            "actions",
            data=np.random.randint(0, 2, (num_samples, max_ships, num_actions)).astype(
                np.uint8
            ),
        )
        f.create_dataset(
            "action_masks",
            data=np.ones((num_samples, max_ships, num_actions), dtype=bool),
        )
