...
def test_logging_correctness():
    ...
    prev_actions = torch.randint(
        0, 3, (B, T, N, 3), device=device
    ).float()  # embedding expects float/int? check code.
    ...
    pred_rewards_from_forward = out[
        6
    ]  # YemongDynamics scaffolds now extracts out[6] as pred_rewards.
    ...
    metrics = model.get_loss(
        ...
        weights_power=weights_p,
        weights_turn=weights_t,
        weights_shoot=weights_s,
    ...
    expected_keys = [
        ...
        "loss_sub/state_DY",
        "loss_sub/state_DVX",
        "loss_sub/action_power",
        "loss_sub/action_turn",
        "loss_sub/action_shoot",
