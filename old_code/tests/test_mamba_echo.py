...
def test_mamba_sanity_echo(device):
    ...
    actor_cache = None
    inference_params = (
        InferenceParams(max_seqlen=T, max_batch_size=B * N) if InferenceParams else None
    )
    ...
    if diff > limit:
        print(
            "FAILURE: Outputs generated in parallel do not match recurrent generation."
        )
        print(
            f"Indices of mismatch: {torch.where((train_logits - inf_logits_cat).abs() > limit)}"
        )
    else:
