    ...
    cfg = OmegaConf.create(
        {
            "model": {
                "noise_scale": 0.1,
                "validation": {"max_batches": 10, "heavy_eval_freq": 2},
            },
            "train": {"amp": False, "epochs": 10, "batch_ratio": 4},
            "wandb": {
                "enabled": True,
            ...
            "environment": {"world_size": [1000.0, 1000.0]},
        }
    )
...
def test_heavy_eval_freq(mock_components):
    ...
    with (
        patch("torch.save"),
        patch(
            "boost_and_broadside.train.world_model.trainer.calculate_action_counts",
            return_value={"power": np.ones(3), "turn": np.ones(7), "shoot": np.ones(2)},
        ),
        patch(
            "boost_and_broadside.train.world_model.trainer.compute_class_weights",
            return_value=torch.ones(3),
        ),
        patch(
            "boost_and_broadside.train.world_model.trainer.apply_turn_exceptions",
            side_effect=lambda x: x,
        ),
        patch(
            "boost_and_broadside.train.world_model.trainer.normalize_weights",
            side_effect=lambda w, c: w,
        ),
    ):
...
def test_logger_heatmap_logic(mock_components):
    ...
    with (
        ...
        patch("wandb.plot.line") as mock_wandb_line,
    ):
        epoch_metrics = {
            "epoch": 2,
            "global_step": 100,
