...
def dummy_h5_data(tmp_path):
    ...
    ep_ids = np.concatenate(
        [np.full((ep1_len,), 1, dtype=np.int32), np.full((ep2_len,), 2, dtype=np.int32)]
    )
    ...
    with h5py.File(file_path, "w") as f:
        ...
        f.create_dataset(
            "rewards", data=np.zeros((total_steps, num_ships), dtype=np.float32)
        )
        f.create_dataset(
            "returns", data=np.zeros((total_steps, num_ships), dtype=np.float32)
        )
...
def test_cross_episode_slice(dummy_h5_data):
    ...
    assert torch.allclose(slice_data_f[0, 0, StateFeature.HEALTH], torch.tensor(100.0))
    assert torch.allclose(slice_data_f[1, 0, StateFeature.HEALTH], torch.tensor(100.0))
    assert torch.allclose(slice_data_f[2, 0, StateFeature.HEALTH], torch.tensor(200.0))
    assert torch.allclose(slice_data_f[3, 0, StateFeature.HEALTH], torch.tensor(200.0))
...
def test_data_loader_pipeline(dummy_h5_data):
    ...
    loader, _ = create_continuous_data_loader(
        dummy_h5_data, batch_size=2, seq_len=5, validation_split=0.0, num_workers=0
    )
