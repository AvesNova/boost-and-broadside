...
from boost_and_broadside.models.yemong.scaffolds import (
    YemongFull,
    YemongSpatial,
    YemongTemporal,
)
...
def test_yemong_full_forward_and_loss():
    ...
    if torch.isnan(state_pred).any():
        print("DEBUG: state_pred has NaNs")
    if torch.isnan(action_logits).any():
        print("DEBUG: action_logits has NaNs")
    if torch.isnan(value_pred).any():
        print("DEBUG: value_pred has NaNs")
...
def test_yemong_spatial_forward_and_loss():
    ...
    _, action_logits, _, _, _ = model(state=state, pos=pos, vel=vel, att=att)
...
def test_yemong_temporal_forward_and_loss():
    ...
    # Forward
    state_pred, _, _, _, _ = model(state=state, prev_action=prev_action)
