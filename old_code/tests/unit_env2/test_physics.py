...
class TestPhysics:
    ...
    def create_state(self, config):
        ...
        return TensorState(
            ...
            ship_pos=torch.zeros(
                (num_envs, max_ships), dtype=torch.complex64, device=device
            ),
            ...
            ship_ang_vel=torch.zeros(
                (num_envs, max_ships), dtype=torch.float32, device=device
            ),
            ship_health=torch.ones(
                (num_envs, max_ships), dtype=torch.float32, device=device
            )
            * config.max_health,
            ship_power=torch.ones(
                (num_envs, max_ships), dtype=torch.float32, device=device
            )
            * config.max_power,
            ...
            ship_team_id=torch.zeros(
                (num_envs, max_ships), dtype=torch.int32, device=device
            ),
            ship_alive=torch.ones(
                (num_envs, max_ships), dtype=torch.bool, device=device
            ),
            ship_is_shooting=torch.zeros(
                (num_envs, max_ships), dtype=torch.bool, device=device
            ),
            prev_action=torch.zeros(
                (num_envs, max_ships, 3), dtype=torch.float32, device=device
            ),
            bullet_pos=torch.zeros(
                (num_envs, max_ships, max_bullets), dtype=torch.complex64, device=device
            ),
            bullet_vel=torch.zeros(
                (num_envs, max_ships, max_bullets), dtype=torch.complex64, device=device
            ),
            bullet_time=torch.zeros(
                (num_envs, max_ships, max_bullets), dtype=torch.float32, device=device
            ),
            bullet_active=torch.zeros(
                (num_envs, max_ships, max_bullets), dtype=torch.bool, device=device
            ),
            bullet_cursor=torch.zeros(
                (num_envs, max_ships), dtype=torch.long, device=device
            ),
        )
    ...
    def test_thrust(self, state, config):
        ...
        assert torch.allclose(
            state.ship_vel[0, 0].real,
            torch.tensor(expected_vel0, dtype=torch.float32),
            atol=1e-5,
        )
        assert torch.allclose(
            state.ship_vel[1, 0].real,
            torch.tensor(expected_vel1, dtype=torch.float32),
            atol=1e-5,
        )
    ...
    def test_shoot(self, state, config):
        ...
        assert torch.allclose(
            ...
            torch.tensor(expected_power, dtype=torch.float32),
            atol=1e-4,
        )
    ...
    def test_collisions(self, state, config):
        ...
        expected_damage = config.bullet_damage * (
            1.0 - 0.9 * torch.exp(torch.tensor(-4.0 * torch.pi))
        )
        assert (
            abs(
                state.ship_health[0, 0].item()
                - (config.max_health - expected_damage.item())
            )
            < 0.01
        ), (
