...
def main(cfg: DictConfig):
    ...
    default_v = torch.tensor(
        StochasticAgentConfig.default_vector(), device=device, dtype=torch.float32
    )
    ...
    print(
        f"{'Matches/Eval':>15} | {'Mean':>8} | {'Std Dev of Mean':>16} | {'95% CI Half-Width'}"
    )
    print("-" * 65)
    for m in [1, 2, 3, 4, 5, 8, 10, 16, 20, 32, 64, 128]:
        ...
        ci_95 = 1.96 * std_eval
        print(
            f"{m:15d} | {np.mean(means):8.4f} | +/- {std_eval:12.4f} | +/- {ci_95:.4f}"
        )
