...
def smooth_data(data, window_length=51, polyorder=3):
    ...
    try:
        ...
        return (
            data.rolling(window=window_length, center=True, min_periods=1).mean().values
        )
...
def analyze_lr_test(csv_path, args=None):
    ...
    smoothed_losses = smooth_data(
        losses, window_length=window_size, polyorder=polyorder
    )
    ...
    if args.max_lr:
        ...
        min_grad_full_idx = np.where(lrs == steepest_lr)[0][
            0
        ]  # Find index in full array for plotting point
    ...
    plt.scatter(
        steepest_lr,
        smoothed_losses[min_grad_full_idx],
        color="red",
        s=100,
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Analyze Learning Rate Range Test results."
    )
    parser.add_argument("csv_path", nargs="?", help="Path to training_log.csv")
    parser.add_argument(
        "--latest", action="store_true", help="Find the latest run automatically"
    )
    parser.add_argument(
        "--max_lr",
        type=float,
        help="Limit analysis to LRs less than or equal to this value",
    )
        ...
        if base_dir.exists():
            ...
            if runs:
                ...
                if possible_path.exists():
                    ...
                    print(
                        f"No training_step_log.csv found in {latest_run}, checking for training_log.csv"
                    )
                    # Fallback
