...
def compute_stats_vectorized(data: torch.Tensor, max_samples: int = 1000000) -> dict:
    ...
    with torch.no_grad():
        ...
        q = torch.quantile(
            data_subset.to(torch.float64),
            torch.tensor([0.25, 0.5, 0.75], device=data.device, dtype=torch.float64),
            dim=0,
        )
...
def main():
    ...
    # Raw State (T, N, 5)
    raw_states = torch.stack([health, power, vel[..., 0], vel[..., 1], ang], dim=-1)
    ...
    sum_d_vel = raw_d_state[..., StateFeature.VX : StateFeature.VY + 1].sum(
        dim=1, keepdim=True
    )  # (T, 1, 2)
    mean_others_d_pos = (sum_d_pos - d_pos) / (N - 1 + 1e-6)
    mean_others_d_vel = (
        sum_d_vel - raw_d_state[..., StateFeature.VX : StateFeature.VY + 1]
    ) / (N - 1 + 1e-6)

    raw_targets[..., TargetFeature.DREL_X : TargetFeature.DREL_Y + 1] = (
        mean_others_d_pos - d_pos
    )
    raw_targets[..., TargetFeature.DREL_VX] = (
        mean_others_d_vel - raw_d_state[..., StateFeature.VX : StateFeature.VY + 1]
    )[..., 0]
    raw_targets[..., TargetFeature.DREL_VY] = (
        mean_others_d_vel - raw_d_state[..., StateFeature.VX : StateFeature.VY + 1]
    )[..., 1]
    ...
    for i in range(0, num_steps, batch_size):
        ...
        with torch.no_grad():
            ...
            # (1, T, N, N, 64) -> (T*N*N, 64)
            rel_flat = chunk_rel[0, : end_idx - i, ..., :num_analytic].reshape(
                -1, num_analytic
            )
        ...
        with open(path, "w") as f:
            ...
            for prefix, s, names in datasets:
                for i, name in enumerate(names):
                    ...
                    f.write(
                        f"{feat_name}|{s['mean'][i]:.6f}|{s['std'][i]:.6f}|{s['rms'][i]:.6f}|{s['sem'][i]:.8f}|{s['min'][i]:.6f}|{s['q1'][i]:.6f}|{s['median'][i]:.6f}|{s['q3'][i]:.6f}|{s['max'][i]:.6f}\n"
                    )
