...
def evaluate_rollouts(
    data_path, dataset_name, model, device, max_steps=50, num_episodes=5
):
    """
    ...
    # Create loader
    train_loader_short, train_loader_long, val_loader_short, val_loader_long = (
        create_unified_data_loaders(
            data,
            short_batch_size=1,
            long_batch_size=1,
    ...
    with torch.no_grad():
        for i in range(num_episodes):
            ...
            episode_errors = []
            inference_params = (
                InferenceParams(max_batch_size=B * N, max_seqlen=max_steps + 10)
                if InferenceParams
                else None
            )
            actor_cache = None
            ...
            for t in range(max_steps):
                ...
                mse = ((next_s_pred - gt_next_s) ** 2 * mask).sum() / (
                    mask.sum() + 1e-6
                )
            ...
            tf_prev_actions = torch.cat(
                [torch.zeros(B, 1, N, 3, device=device), tf_actions_data], dim=1
            )
            ...
            for t in range(max_steps):
                step_mse = (
                    (pred_s_tf[:, t] - gt_next_s_tf[:, t]) ** 2 * mask_tf[:, t]
                ).sum() / (mask_tf[:, t].sum() + 1e-6)
...
def analyze_feature_errors(data_path, model, device, max_steps=50, num_episodes=20):
    ...
    return (
        sum_err.cpu().numpy(),
        sum_sq_err.cpu().numpy(),
        sum_abs_err.cpu().numpy(),
        sum_delta.cpu().numpy(),
        sum_sq_delta.cpu().numpy(),
        total_count.cpu().numpy(),
    )
...
def main():
    ...
    parser.add_argument(
        "--run_dir", type=str, help="Run directory (e.g. models/world_model/run_...)"
    )
    ...
    mamba_cfg = MambaConfig(
        ...
        action_dim=12,
        target_dim=9,
        loss_type=getattr(cfg.world_model, "loss_type", "fixed"),
        max_context_len=int(cfg.world_model.seq_len),
    )
    ...
    if "state_encoder.0.weight" in new_state_dict:
        ...
        if ckpt_dim != mamba_cfg.input_dim:
            ...
            if "world_head.3.weight" in new_state_dict:
                mamba_cfg.target_dim = new_state_dict["world_head.3.weight"].shape[0]
            model = MambaBB(mamba_cfg).to(device)
    ...
    results = {}
    bc_data_path = (
        cfg.train.bc_data_path
        if cfg.train.bc_data_path and os.path.exists(cfg.train.bc_data_path)
        else None
    )
    if not bc_data_path:
        import glob

        h5_files = glob.glob("data/**/*.h5", recursive=True) + glob.glob(
            "data/**/*.hdf5", recursive=True
        )
    ...
    if bc_data_path:
        d_mse, tf_mse = evaluate_rollouts(
            bc_data_path, "Validation", model, device, max_steps=50
        )
        if d_mse is not None:
            results["Dataset_Dream"] = d_mse
            results["Dataset_TF"] = tf_mse

    sim_mse, dream_mse = evaluate_live_rollouts(
        model, device, num_scenarios=20, max_steps=50
    )
    ...
    labels = {
        "Dataset_TF": "Dataset: TF",
        "Dataset_Dream": "Dataset: Dream",
        "Live_Sim": "Live: Sim",
        "Live_Dream": "Live: Dream",
    }
    ...
    plt.xlabel("Step")
    plt.ylabel("MSE")
    plt.title("MambaBB Eval")
    plt.legend()
    plt.grid(True, alpha=0.3)
    plt.yscale("log")
    plt.savefig(out_dir / "rollout_eval_full.png")
    plt.close()
    ...
    with open(out_dir / "summary.csv", "w") as f:
        ...
        for key, arr in results.items():
            ...
            f.write(
                f"{labels.get(key, key)},{arr[0]:.6f},{arr[9] if len(arr) > 9 else 0:.6f},{arr[49] if len(arr) > 49 else 0:.6f}\n"
            )

    if bc_data_path:
        sum_err, sum_sq_err, sum_abs_err, sum_delta, sum_sq_delta, t_count = (
            analyze_feature_errors(
                bc_data_path, model, device, max_steps=50, num_episodes=50
            )
        )
        ...
        std_err = np.sqrt(
            np.maximum(0, (sum_sq_err / (t_count + 1e-6)) - (mean_err**2))
        )
        ...
        std_delta = np.sqrt(
            np.maximum(0, (sum_sq_delta / (t_count + 1e-6)) - (mean_delta**2))
        )
        ...
        with open(out_dir / "feature_errors.csv", "w") as f:
            f.write(
                "Feature_ID,Feature_Name,Err_Bias,Err_Std,Err_MAE,Delta_Bias,Delta_Std,Count\n"
            )
            for i in range(len(mean_err)):
                lbl = feature_labels[i] if i < len(feature_labels) else f"Dim_{i}"
                f.write(
                    f"{i},{lbl},{mean_err[i]:.8f},{std_err[i]:.8f},{mae_err[i]:.8f},{mean_delta[i]:.8f},{std_delta[i]:.8f},{int(t_count[i])}\n"
                )
