...
def parse_args():
    ...
    parser.add_argument(
        "--d-state", type=int, default=64, help="SSM state expansion factor for Mamba"
    )
    parser.add_argument(
        "--d-conv", type=int, default=4, help="local convolution width for Mamba"
    )
    parser.add_argument(
        "--expand", type=int, default=2, help="expansion factor for the Mamba block"
    )
    parser.add_argument(
        "--mamba-lr",
        type=float,
...
class Agent(nn.Module):
    def __init__(self, envs, args):
        ...
        if args.gym_id in mujoco_envs:
            ...
            if self.obs_space.shape[0] in [1, 3, 4]:
                in_channels = self.obs_space.shape[
                    0
                ]  # channels-first (e.g., ALE/Breakout-v5)
            else:
        ...
        if self.is_continuous:
            ...
            self.actor_mean = layer_init(
                nn.Linear(args.hidden_dim, action_dim), std=0.01
            )
            self.actor_logstd = nn.Parameter(torch.zeros(1, action_dim))
        else:
            self.actor = layer_init(
                nn.Linear(args.hidden_dim, envs.single_action_space.n), std=0.01
            )

    def get_states(self, x):
        """Process observations to get encoded states."""
        if (
            "minigrid" in self.args.gym_id.lower()
            or "mortar" in self.args.gym_id.lower()
        ):
    ...
    def forward_sequence(self, x, init_mamba_state=None):
        ...
        out = self.mamba(
            features, inference_params=inference_params
    ...
    def get_value(self, x, mamba_state):
        ...
        out, new_conv_state, new_ssm_state = self.mamba.step(
            current, mamba_state[0], mamba_state[1]
        )
...
if __name__ == "__main__":
    ...
    if "ale" in args.gym_id.lower():
        ...
        envs_lst = [
            make_classic_env(
                ...
                args.capture_video,
                run_name,
                masked_indices=args.masked_indices,
            )
            for i in range(args.num_envs)
        ]
    ...
    optimizer = optim.Adam(
        [
            ...
            {"params": agent.critic.parameters()},
            {
                ...
                else list(agent.actor_mean.parameters()) + [agent.actor_logstd]
            },
    ...
    print(
        f"Total parameters: {total_params}, trainable parameters: {trainable_params / 1e6:.4f}M"
    )
    ...
    conv_state, ssm_state = agent.mamba.allocate_inference_cache(
        args.num_envs, max_seqlen=1
    )
    ...
    for update in range(1, num_updates + 1):
        ...
        for step in range(0, args.num_steps):
            ...
            inf_start = time.time()
            with torch.no_grad():
                action, logprob, _, value, next_mamba_state = (
                    agent.get_action_and_value(next_obs, next_mamba_state)
                )
            ...
            if final_info is not None and len(final_info) > 0:
                valid_entries = [
                    ...
                    if entry is not None and "episode" in entry
                ]
                if valid_entries:
                    episodic_returns = [
                        entry["episode"]["r"] for entry in valid_entries
                    ]
                    episodic_lengths = [
                        entry["episode"]["l"] for entry in valid_entries
                    ]
        ...
        writer.add_scalar(
            "metrics/inference_latency", avg_inference_latency, global_step
        )
        ...
        for epoch in range(args.update_epochs):
            ...
            for start in range(0, args.num_envs, envsperbatch):
                ...
                full_seq_output = agent.forward_sequence(
                    mb_obs, init_state
                )  # shape: [T, minibatch_size, hidden_dim]
                T, B, hidden_dim = full_seq_output.shape
                flat_features = full_seq_output.reshape(
                    -1, hidden_dim
                )  # shape: [T*B, hidden_dim]
                ...
                if args.clip_vloss:
                    ...
                    v_clipped = mb_values_flat + torch.clamp(
                        new_values - mb_values_flat, -args.clip_coef, args.clip_coef
                    )
        ...
        current_return = (
            np.mean([ep["r"] for ep in episode_infos]) if episode_infos else 0.0
        )
        ...
        writer.add_scalar(
            "charts/mamba_learning_rate", optimizer.param_groups[-1]["lr"], global_step
        )
        ...
        if episode_infos:
            ...
            writer.add_scalar(
                "charts/avg_episode_return", avg_episode_return, global_step
            )
        ...
        if torch.cuda.is_available():
            ...
            gpu_memory_allocated_percent = (
                gpu_memory_allocated / total_gpu_memory
            ) * 100
            gpu_memory_reserved_percent = (gpu_memory_reserved / total_gpu_memory) * 100

            writer.add_scalar(
                "metrics/GPU_memory_allocated_GB", gpu_memory_allocated_gb, global_step
            )
            writer.add_scalar(
                "metrics/GPU_memory_reserved_GB", gpu_memory_reserved_gb, global_step
            )
            writer.add_scalar(
                "metrics/GPU_memory_allocated_percent",
                gpu_memory_allocated_percent,
                global_step,
            )
            writer.add_scalar(
                "metrics/GPU_memory_reserved_percent",
                gpu_memory_reserved_percent,
                global_step,
            )
        ...
        if args.save_model and update % args.save_interval == 0:
            model_path = (
                f"runs/{run_name}/{args.exp_name}_update_{update}.cleanrl_model"
            )
            model_data = {
