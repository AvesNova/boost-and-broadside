...
class GameRenderer:
    ...
    data structures from `boost_and_broadside.core.types`.
    """
    ...
    def __init__(self, config: ShipConfig, target_fps: int = 60):
        ...
        try:
            ...
            if "windows not available" in str(
                e
            ) or "video system not initialized" in str(e):
                print("\n" + "=" * 80)
                print(
                    "CRITICAL ERROR: Pygame failed to open a window (Headless environment detected)."
                )
                print(
                    "To fix this, append 'environment.render_mode=none' to your command."
                )
    ...
    def handle_events(self):
        ...
        for ship_id in self.human_players:
            self.human_actions[ship_id] = {"power": power, "turn": turn, "shoot": shoot}
    ...
    def render(self, state: RenderState):
        ...
        scaled_surface = pygame.transform.smoothscale(
            self.display_surface, (new_width, new_height)
        )
    ...
    def _draw_grid(self):
        ...
        for x in range(0, int(self.world_size[0]) + 1, 100):
            ...
            pygame.draw.line(
                self.display_surface, GRID_COLOR, (sx, 0), (sx, self.game_height)
            )
        ...
        for y in range(0, int(self.world_size[1]) + 1, 100):
            ...
            pygame.draw.line(
                self.display_surface, GRID_COLOR, (0, sy), (self.game_width, sy)
            )
    ...
    def _draw_ship(self, ship: RenderShip):
        ...
        pygame.draw.rect(
            self.display_surface, HP_BAR_BG, (bar_x, bar_y, bar_width, bar_height)
        )
        pygame.draw.rect(
    ...
    def _draw_ui(self, state: RenderState):
        ...
        sidebar_rect = pygame.Rect(
            self.game_width, 0, self.sidebar_width, self.native_height
        )
        ...
        pygame.draw.line(
            ...
            (100, 100, 100),
            (self.game_width, 0),
            (self.game_width, self.native_height),
        )
        ...
        for ship_id in sorted(state.ships.keys()):
            ...
            for line in stats:
                text = self.font.render(
                    line, True, color if line == stats[0] else TEXT_COLOR
                )
