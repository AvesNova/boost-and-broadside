...
class BatchedEvolvableAgent:
    ...
    def __init__(self, ship_config: ShipConfig):
        ...
        self.flat_action_sampling = (
            False  # Match stochastic_config default or set it manually
        )
        ...
        self.bounds_diff = torch.tensor(
            [b[1] - b[0] for b in bounds], dtype=torch.float32
        )
    ...
    def _linear_ramp(
        ...
        low: torch.Tensor,
        high: torch.Tensor,
        prob_lo: torch.Tensor,
        prob_hi: torch.Tensor,
    ) -> torch.Tensor:
        """
        x: (B, N)
    ...
    def _compute_action_probs(
        ...
        bounds_diff = (
            self.bounds_diff.to(device).view(1, 1, 12).repeat_interleave(2, dim=2)
        )
        ...
        power_probs = power_probs / (
            torch.sum(power_probs, dim=-1, keepdim=True) + 1e-8
        )
        ...
        shoot_threshold = torch.clamp(
            target_angular_size, np.deg2rad(1.0), np.deg2rad(45.0)
        )
        ...
        shoot_probs = shoot_probs / (
            torch.sum(shoot_probs, dim=-1, keepdim=True) + 1e-8
        )
    ...
    def get_actions(
        self, state: TensorState, config_tensor: torch.Tensor
    ) -> torch.Tensor:
        """
        ...
        p_power, p_turn, p_shoot = self._compute_action_probs(
            state, config_tensor, closest_dist, dir_pred, active_mask
        )
