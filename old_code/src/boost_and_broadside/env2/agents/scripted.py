...
class VectorScriptedAgent:
    ...
    def _compute_actions(
        ...
        actions_power = torch.zeros(
            (batch_size, num_ships), dtype=torch.long, device=device
        )
        actions_turn = torch.zeros(
            (batch_size, num_ships), dtype=torch.long, device=device
        )
        actions_shoot = torch.zeros(
            (batch_size, num_ships), dtype=torch.long, device=device
        )
        ...
        want_boost = (
            power_ratio > 0.9 * (1.0 - closest_dist / self.max_shooting_range)
        ) | (speed < 30.0)
        ...
        shoot_threshold = torch.clamp(
            shoot_threshold, np.deg2rad(1.0), np.deg2rad(45.0)
        )
        ...
        actions_power = torch.where(
            valid_action_mask, actions_power, torch.tensor(0, device=device)
        )
        actions_turn = torch.where(
            valid_action_mask, actions_turn, torch.tensor(0, device=device)
        )
        actions_shoot = torch.where(
            valid_action_mask, actions_shoot, torch.tensor(0, device=device)
        )
