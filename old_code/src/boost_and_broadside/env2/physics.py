...
def _handle_shooting(
    ...
    state.bullet_time[batch_indices, ship_indices, bullet_slot_indices] = (
        config.bullet_lifetime
    )
    state.bullet_active[batch_indices, ship_indices, bullet_slot_indices] = True

    state.bullet_cursor[batch_indices, ship_indices] = (
        bullet_slot_indices + 1
    ) % state.max_bullets
...
def _apply_combat_damage(
    ...
    flat_bullets_pos = state.bullet_pos.view(
        batch_size, num_ships * num_bullets_per_ship
    )
    flat_bullets_active = state.bullet_active.view(
        batch_size, num_ships * num_bullets_per_ship
    )

    # Calculate wrapped difference between ships and bullets
    # diff: Vector from bullet to ship
    diff = state.ship_pos.unsqueeze(2) - flat_bullets_pos.unsqueeze(
        1
    )  # (B, N_ships, N_total_bullets)
    diff.real = (diff.real + world_width / 2) % world_width - world_width / 2
    diff.imag = (diff.imag + world_height / 2) % world_height - world_height / 2
    ...
    hit_mask = (
        ...
        & flat_bullets_active.unsqueeze(1)
        & state.ship_alive.unsqueeze(2)
    )

    # Ignore own bullets (ships cannot shoot themselves)
    bullet_source_idx = (
        torch.arange(num_ships * num_bullets_per_ship, device=device)
        // num_bullets_per_ship
    )
    own_bullet_mask = bullet_source_idx.view(1, 1, -1) == torch.arange(
        num_ships, device=device
    ).view(1, num_ships, 1)
    ...
    flat_bullets_vel = state.bullet_vel.view(
        batch_size, num_ships * num_bullets_per_ship
    )
    ...
    state.ship_health = torch.maximum(
        state.ship_health, torch.tensor(0.0, device=device)
    )

    # Calculate Rewards based on whom hit whom - REMOVED (Handled by RewardFunction)

    # Deactivate bullets that hit
    bullet_hits_flat = valid_hit.any(
        dim=1
    )  # (B, N_total_bullets) - True if bullet hit ANY ship
