...
def tensor_state_to_render_state(
    tensor_state: TensorState, config: ShipConfig, batch_idx: int = 0
) -> RenderState:
    """
    ...
    # Derive teams/owners
    ship_ids_matrix = (
        torch.arange(num_ships, device=tensor_state.device)
        .unsqueeze(1)
        .expand_as(tensor_state.bullet_pos[batch_idx])
    )
