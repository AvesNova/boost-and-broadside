...
class TensorEnv:
    ...
    def _initialize_state(self):
        ...
        self.state = TensorState(
            ...
            ship_pos=torch.zeros(
                (self.num_envs, self.max_ships),
                dtype=torch.complex64,
                device=self.device,
            ),
            ship_vel=torch.zeros(
                (self.num_envs, self.max_ships),
                dtype=torch.complex64,
                device=self.device,
            ),
            ship_attitude=torch.zeros(
                (self.num_envs, self.max_ships),
                dtype=torch.complex64,
                device=self.device,
            ),
            ship_ang_vel=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.float32, device=self.device
            ),
            ship_health=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.float32, device=self.device
            ),
            ship_power=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.float32, device=self.device
            ),
            ship_cooldown=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.float32, device=self.device
            ),
            ship_team_id=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.int32, device=self.device
            ),
            ship_alive=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.bool, device=self.device
            ),
            ship_is_shooting=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.bool, device=self.device
            ),
            prev_action=torch.zeros(
                (self.num_envs, self.max_ships, 3),
                dtype=torch.float32,
                device=self.device,
            ),
            bullet_pos=torch.zeros(
                (self.num_envs, self.max_ships, self.max_bullets),
                dtype=torch.complex64,
                device=self.device,
            ),
            ...
            bullet_cursor=torch.zeros(
                (self.num_envs, self.max_ships), dtype=torch.long, device=self.device
            ),
        )
    ...
    def step(
        ...
        if dones.any():
            ...
            info["final_observation"] = {
                k: v.clone() for k, v in self._get_obs().items()
            }
    ...
    def _reset_envs(
        self, env_mask: torch.Tensor, options: Optional[Dict[str, Any]] = None
    ):
        """
        ...
        # Random Positions
        rand_x = (
            torch.rand((num_resets, self.max_ships), device=self.device) * world_width
        )
        rand_y = (
            torch.rand((num_resets, self.max_ships), device=self.device) * world_height
        )
        ...
        # Random Attitude
        rand_angle = (
            torch.rand((num_resets, self.max_ships), device=self.device) * 2 * np.pi
        )
        ...
        if self.config.random_speed:
            ...
            # Scale to [min, max]
            speed = self.config.min_speed + rand_speed * (
                self.config.max_speed - self.config.min_speed
            )
        else:
            speed = torch.full(
                (num_resets, self.max_ships),
        ...
        # Defaults
        new_team_ids = torch.zeros(
            (num_resets, self.max_ships), dtype=torch.int32, device=self.device
        )
        new_alive = torch.zeros(
            (num_resets, self.max_ships), dtype=torch.bool, device=self.device
        )
    ...
    def _get_obs(self) -> Dict[str, torch.Tensor]:
        ...
        ship_ids = torch.arange(num_ships, device=self.device).expand(
