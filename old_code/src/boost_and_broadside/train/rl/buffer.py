    ...
    def __init__(
        ...
        for key, shape in obs_shapes.items():
            self.obs[key] = torch.zeros(
                (num_steps, num_envs, *shape), device=device, dtype=torch.float32
            )

        # Actions: (T, B, N, 3)
        # action_shape passed in is (N, 3)
        self.actions = torch.zeros(
            (num_steps, num_envs, *action_shape), device=device, dtype=torch.float32
        )

        # Per-Ship Scalars
        self.logprobs = torch.zeros(
            (num_steps, num_envs, self.num_ships), device=device, dtype=torch.float32
        )
        self.rewards = torch.zeros(
            (num_steps, num_envs, 1), device=device, dtype=torch.float32
        )  # team scalar
