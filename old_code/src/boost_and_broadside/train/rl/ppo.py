...
class PPOTrainer:
    def __init__(self, cfg: DictConfig, env, agent):
        ...
        self.optimizer = optim.Adam(
            self.agent.parameters(), lr=cfg.train.ppo.learning_rate, eps=1e-5
        )
    ...
    def train(self):
        ...
        for iteration in range(1, self.num_iterations + 1):
            ...
            for l_idx, (conv, ssm) in next_mamba_state.items():
                ...
                c_vs = conv.view(
                    self.cfg.train.ppo.num_envs, self.max_ships, *conv.shape[1:]
                )
                # ssm: (B*N, D, N) -> (B, N, D, N)
                s_vs = ssm.view(
                    self.cfg.train.ppo.num_envs, self.max_ships, *ssm.shape[1:]
                )
            ...
            for step in range(0, self.cfg.train.ppo.num_steps):
                ...
                with torch.no_grad():
                    ...
                    if getattr(self.agent, "is_interleaved", False):
                        ...
                        action, logprob, _, value, next_mamba_state, *_ = (
                        ...
                        action, logprob, _, value, next_mamba_state, *_ = (
                            self.agent.get_action_and_value(obs_input, next_mamba_state)
                        )
                        action_sq = action.squeeze(1)
                        logprob_sq = logprob.squeeze(1)
                        value_sq = value.squeeze(1)
                ...
                # Execute Step
                next_obs_new, reward, terminated, truncated, info = self.env.step(
                    action_sq
                )
                ...
                # Buffer Add
                self.buffer.add(
                    next_obs, action_sq, logprob_sq, reward, next_done, value_sq
                )
            ...
            with torch.no_grad():
                obs_input = {k: v.unsqueeze(1) for k, v in next_obs.items()}
                _, _, _, next_value, _ = self.agent.get_action_and_value(
            ...
            y_pred, y_true = (
                self.buffer.values.flatten().cpu().numpy(),
                self.buffer.returns.flatten().cpu().numpy(),
            )
            var_y = np.var(y_true)
            explained_var = (
            ...
            for epoch in range(self.cfg.train.ppo.update_epochs):
                iterator = self.buffer.get_minibatch_iterator(
                    self.cfg.train.ppo.num_minibatches
                )
                ...
                for (
                    ...
                    obs_input = {
                        k: v.permute(1, 0, *range(2, v.ndim)).contiguous()
                    ...
                    for l_idx, (conv, ssm) in mb_initial_state.items():
                        ...
                        c_flat = conv.reshape(
                            current_batch_size * self.max_ships, *conv.shape[2:]
                        )
                        s_flat = ssm.reshape(
                            current_batch_size * self.max_ships, *ssm.shape[2:]
                        )
                    ...
                    mb_rewards_perm = mb_rewards.permute(
                        1, 0, 2
                    ).contiguous()  # (B, T, N)
                    mb_next_obs_perm = (
                        mb_next_obs["state"].permute(1, 0, 2, 3).contiguous()
                    )  # (B, T, N, F)
                    ...
                    dones_shifted[:, 0] = (
                        0  # First step state is handled by passing 'flat_mamba_state'
                    )
                    ...
                    with torch.no_grad():
                        ...
                        clipfracs += [
                            ((ratio - 1.0).abs() > self.cfg.train.ppo.clip_coef)
                            .float()
                            .mean()
                            .item()
                        ]
                    ...
                    mb_curr_obs_perm = (
                        mb_obs["state"].permute(1, 0, 2, 3).contiguous()
                    )  # (B, T, N, F)
                    mb_next_obs_perm = (
                        mb_next_obs["state"].permute(1, 0, 2, 3).contiguous()
                    )
                    ...
                    # Masks
                    valid_mask = (
                        1.0 - mb_dones_perm.float()
                    )  # (B, T), zero at done steps
                    ...
                    loss_out = self.agent.get_loss(
                        ...
                        lambda_state=self.cfg.train.ppo.get(
                            "lambda_state", 0.5
                        ),  # Configurable RL weights
                        lambda_actions=self.cfg.train.ppo.get(
                            "lambda_actions", 0.0
                        ),  # Off by default
                        pred_values=new_value.unsqueeze(-1)
                        if new_value.ndim == 2
                        else new_value,
                        pred_rewards=reward_pred,
                        target_returns=mb_returns[..., 0:1],
                        target_rewards=mb_rewards_perm[..., 0:1],
                        lambda_value=self.cfg.train.ppo.get(
                            "lambda_value", self.cfg.train.ppo.vf_coef
                        ),
                    ...
                    loss = (
                        pg_loss
                        - self.cfg.train.ppo.ent_coef * entropy_loss
                        + loss_out["loss"]
                    )
                    ...
                    nn.utils.clip_grad_norm_(
                        self.agent.parameters(), self.cfg.train.ppo.max_grad_norm
                    )
                    self.optimizer.step()
                ...
                if (
                    self.cfg.train.ppo.target_kl is not None
                    and approx_kl > self.cfg.train.ppo.target_kl
                ):
                    break
            ...
            if iteration % self.cfg.wandb.log_interval == 0:
                ...
                # Mean Losses
                mean_losses = {
                    k: np.mean(v) if len(v) > 0 else 0.0 for k, v in loss_stats.items()
                }
                mean_clipfrac = np.mean(clipfracs) if len(clipfracs) > 0 else 0.0

                # Log Losses
                self.writer.add_scalar(
                    "losses/policy_loss", mean_losses["pg_loss"], global_step
                )
                self.writer.add_scalar(
                    "losses/value_loss", mean_losses["v_loss"], global_step
                )
                self.writer.add_scalar(
                    "losses/entropy", mean_losses["entropy_loss"], global_step
                )
                self.writer.add_scalar(
                    "losses/old_approx_kl", mean_losses["old_approx_kl"], global_step
                )
                self.writer.add_scalar(
                    "losses/approx_kl", mean_losses["approx_kl"], global_step
                )
                ...
                self.writer.add_scalar(
                    "losses/state_loss", mean_losses["s_loss"], global_step
                )
                self.writer.add_scalar(
                    "losses/reward_loss", mean_losses["r_loss"], global_step
                )
                ...
                if len(iter_episode_returns) > 0:
                    ...
                    self.writer.add_scalar(
                        "charts/episodic_return", mean_ep_ret, global_step
                    )
                    self.writer.add_scalar(
                        "charts/episodic_length", mean_ep_len, global_step
                    )
                else:
                ...
                print(
                    f"Iteration {iteration}: SPS={sps} P_Loss={mean_losses['pg_loss']:.3f} V_Loss={mean_losses['v_loss']:.3f} S_Loss={mean_losses['s_loss']:.3f} R_Loss={mean_losses['r_loss']:.3f} Ep_Ret={mean_ep_ret:.1f}"
                )
