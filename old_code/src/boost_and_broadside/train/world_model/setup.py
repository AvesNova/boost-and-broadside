...
def automate_num_reward_components(cfg: DictConfig):
    ...
    if "rewards" in cfg:
        ...
        log.info(
            f"Implicitly setting num_reward_components = {num_rewards} from 'rewards' config."
        )
        ...
        if "loss" in cfg.model and "losses" in cfg.model.loss:
            for loss_cfg in cfg.model.loss.losses:
                if "RewardLoss" in loss_cfg._target_:
                    ...
                    log.info(
                        f"Injected reward names into RewardLoss config: {reward_names}"
                    )
...
def create_model(cfg: DictConfig, data_path: str, device: torch.device):
    ...
    log.info(
        f"Instantiating model with input_dim={cfg.model.get('input_dim')} and target_dim={cfg.model.get('target_dim')}"
    )
...
def create_scheduler(
    optimizer: optim.Optimizer, cfg: DictConfig, total_steps: int
    ...
    if "range_test" in sched_cfg.type:
        ...
        log.info(
            f"Initializing Exponential Range Test Scheduler: {range_cfg.start_lr} -> {range_cfg.end_lr} over {total_steps} steps"
        )
        ...
        lr = cfg.train.get("learning_rate", cfg.model.get("learning_rate", 1e-4))
        log.info(
            f"Initializing Warmup Constant Scheduler: {warmup_cfg.start_lr} -> {lr} over {warmup_cfg.steps} steps"
        )
        ...
        def lr_lambda(step):
            if step < warmup_steps:
                return (
                    start_lr + (target_lr - start_lr) * (step / warmup_steps)
                ) / target_lr
            return 1.0
