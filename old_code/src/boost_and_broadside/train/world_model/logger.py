...
class MetricLogger:
    def __init__(self, cfg: DictConfig, run_dir: Path):
        ...
        self.is_range_test = sched_cfg and "range_test" in getattr(
            sched_cfg, "type", ""
        )
    ...
    def log_epoch(self, epoch_metrics: dict, epoch: int):
        ...
        if swa_loss == "":
            swa_loss = ""  # Ensure empty string if missing
        else:
            swa_loss = f"{swa_loss:.6f}"
        ...
        if self.wb_cfg and self.wb_cfg.get("enabled", False):
            ...
            wandb_log = {
                k: v
                for k, v in epoch_metrics.items()
                if "conf_mat" not in k and "Loss/" not in k and "Train/" not in k
            }
            ...
            wandb.log(
                wandb_log, step=epoch_metrics.get("global_step", epoch * 100)
            )  # Fallback step if missing
