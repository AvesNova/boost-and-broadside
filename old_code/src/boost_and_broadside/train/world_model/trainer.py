...
from boost_and_broadside.utils.dataset_stats import (
    calculate_action_counts,
    compute_class_weights,
    apply_turn_exceptions,
    normalize_weights,
    calculate_flattened_action_counts,
)
...
class Trainer:
    def __init__(
        ...
        self.epochs = cfg.train.get(
            "epochs", 1000
        )  # Moved to train section or model? Usually train.
        ...
        self.epochs = (
            cfg.train.get("epochs", 1000)
            if "epochs" in cfg.train
            else cfg.model.get("epochs", 1000)
        )
        ...
        self.current_min_skill = (
            self.curr_cfg.get("min_skill_start", 0.0)
        ...
        print(
            f"DEBUG: Trainer init - Device: {device}, Use AMP: {self.use_amp}, Dtype: {dtype}"
        )
        ...
        w_turn_full = apply_turn_exceptions(
            compute_class_weights(counts["turn"], cap=w_cap, power=w_pwr)
        )
                    ...
                    pbar.set_postfix(
                        {
                            ...
                            "state": (
                                accumulators["total_state_loss"].item() / steps_in_epoch
                            )
                            if steps_in_epoch > 0
                            else 0,
                        }
    ...
    def _train_step(self, batch_data, epoch, params):
        ...
        processed = BatchProcessor.process_batch(
            batch_data, self.cfg, self.device, use_amp=self.use_amp
        )
        ...
        with torch.amp.autocast(
            ...
            if use_soft_bins and soft_bin_logits is not None:
                ...
                next_states_vel = processed["inputs"][
                    "state"
                ]  # (B,T,N,D) — this is input states
                ...
                states_full = batch_data["states"].to(
                    self.device, dtype=torch.float32, non_blocking=True
                )
                pos_full = batch_data["pos"].to(
                    self.device, dtype=torch.float32, non_blocking=True
                )
    ...
    def _update_accumulators(self, acc, metrics):
        # Epoch Totals
        acc["total_loss"] += metrics[
            "loss"
        ]  # No need to undo div; loss is not scaled here
        ...
        m = metrics["metrics"]
        for k, v in m.items():
            if k not in acc["acc_errors"]:
                acc["acc_errors"][k] = 0.0
            acc["acc_errors"][k] += v
    ...
    def _optimize_step(self, acc, pbar, is_range_test, epoch):
        ...
        step_time = acc[
            "acc_time"
        ]  # Pure compute time for this step (accumulated across micro-steps)
        ...
        metrics = {
            ...
            "param/entropy_lambda": self._get_current_params(self.global_step)[
                "lambda_entropy"
            ],
            "param/focal_gamma": self._get_current_params(self.global_step)[
                "focal_gamma"
            ],
            "param/min_skill": self._get_current_params(self.global_step)["min_skill"],
            "time/micro_batch": acc["acc_time"]
            / self.acc_steps,  # Avg pure compute time per micro step
    ...
    def _log_epoch_summary(self, acc, steps, epoch):
        ...
        current_lr = self.optimizer.param_groups[0]["lr"]
        log.info(
            f"Epoch {epoch + 1}: LR={current_lr:.2e} Train Loss={avg_loss:.4f} (State={avg_state:.4f}, Action={avg_action:.4f}, Rel={avg_rel:.4f}, Val={avg_value:.4f}, Rew={avg_reward:.4f})"
        )
    ...
    def _handle_swa(self, epoch, val_loaders):
        ...
        if swa_start_condition:
            ...
            val_metrics_swa = self.validator.validate_validation_set(
                val_loaders, swa_model=self.swa_model.averaged_model
            )
            ...
            if avg_val_loss_swa < self.best_val_loss_swa:
                ...
                torch.save(
                    self.swa_model.averaged_model.state_dict(),
                    self.run_dir / "best_world_model_swa.pth",
                )
                log.info(
                    f"Saved best SWA model with val loss {self.best_val_loss_swa:.4f}"
                )
    ...
    def _save_checkpoint(self, epoch):
        if (epoch + 1) % 10 == 0 or (epoch + 1) == self.epochs:
            ...
            name = (
                f"world_model_epoch_{epoch + 1}.pt"
                if (epoch + 1) < self.epochs
                else "final_world_model.pt"
            )
