...
class UnifiedEpisodeDataset:
    ...
    def __init__(
        self, h5_path: str, world_size: tuple[float, float] = (1024.0, 1024.0)
    ):
        ...
        if len(self.episode_lengths) > 0:
            self.total_timesteps = (
                self.episode_starts[-1].item() + self.episode_lengths[-1].item()
            )
        else:
    ...
    def get_cross_episode_slice(
        self, dataset_name: str, global_start: int, length: int
    ) -> torch.Tensor:
        """
...
class BaseView(Dataset):
    ...
    def _compute_aligned_rewards(
        ...
        rewards_dict = self.reward_registry.compute_all(
            p_state, actions, n_state, dones
        )
        ...
        reward_list = [
            rewards_dict[comp.name] for comp in self.reward_registry.components
        ]
...
class ShortView(BaseView):
    ...
    def __getitem__(
        ...
        seq_target_actions, seq_expert_probs = self._get_target_actions(
            abs_start, abs_end
        )
        ...
        if self.reward_registry:
            seq_rewards = self._compute_aligned_rewards(
                seq_tokens, seq_team_ids, seq_rewards_raw
            )
        else:
        ...
        if actual_len < self.seq_len:
            ...
            pos_pad = torch.zeros(pad_len, *seq_pos.shape[1:], dtype=seq_pos.dtype)
            ...
            seq_target_actions = torch.cat(
                [seq_target_actions, target_action_pad], dim=0
            )
        ...
        return {
            ...
            "expert_action_probs": seq_expert_probs
            if seq_expert_probs is not None
            else torch.empty(0),
            "returns": seq_returns,
            "rewards": seq_rewards,
