...
class ContinuousView(BaseView):
    ...
    def __init__(
        ...
        if not self.dataset.has_dataset("episode_ids"):
            raise ValueError(
                "ContinuousView requires 'episode_ids' in HDF5 for handling state resets."
            )
    ...
    def __getitem__(self, idx: int):
        ...
        # 1. Fetch Data
        tokens = self.dataset.get_cross_episode_slice(
            "tokens", global_start, self.seq_len
        )
        actions = self.dataset.get_cross_episode_slice(
            "actions", global_start, self.seq_len
        )

        target_actions = actions
        if self.dataset.has_dataset("expert_actions"):
            target_actions = self.dataset.get_cross_episode_slice(
                "expert_actions", global_start, self.seq_len
            )

        expert_action_probs = None
        if self.dataset.has_dataset("expert_action_probs"):
            expert_action_probs = self.dataset.get_cross_episode_slice(
                "expert_action_probs", global_start, self.seq_len
            )
        episode_ids = self.dataset.get_cross_episode_slice(
            "episode_ids", global_start, self.seq_len
        )

        if self.dataset.has_dataset("team_ids"):
            team_ids = self.dataset.get_cross_episode_slice(
                "team_ids", global_start, self.seq_len
            )
        else:
        ...
        if self.dataset.has_dataset("action_masks"):
            action_masks = self.dataset.get_cross_episode_slice(
                "action_masks", global_start, self.seq_len
            )
        else:
        ...
        if global_start == 0:
            ...
            prev_id_tensor = self.dataset.get_cross_episode_slice(
                "episode_ids", global_start - 1, 1
            )
            prev_id = prev_id_tensor[0].int().item()

        full_ids = torch.cat(
            [torch.tensor([prev_id], dtype=torch.int32), seq_idx], dim=0
        )
        ...
        pos = self.dataset.get_cross_episode_slice(
            "position", global_start, self.seq_len
        )
        att = self.dataset.get_cross_episode_slice(
            "attitude", global_start, self.seq_len
        )

        # Rewards and Returns
        rewards_raw = self.dataset.get_cross_episode_slice(
            "rewards", global_start, self.seq_len
        )
        returns = self.dataset.get_cross_episode_slice(
            "returns", global_start, self.seq_len
        )

        # Compute Rewards On-The-Fly if configured
        if self.reward_registry:
            rewards = self._compute_aligned_rewards(
                tokens, team_ids, rewards_raw, seq_pos=pos, seq_att=att
            )
        else:
            rewards = rewards_raw
