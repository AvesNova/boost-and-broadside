...
class BatchProcessor:
    ...
    def compute_targets(input_states, next_states, pos_curr, pos_next, W, H, device):
        ...
        target_states = torch.zeros(
            (*d_state.shape[:-1], TARGET_DIM), device=device, dtype=d_state.dtype
        )
    ...
    def process_batch(batch_data, config, device, use_amp=False):
        ...
        if "target_actions" in batch_data:
            target_actions = batch_data["target_actions"].to(device, non_blocking=True)[
                :, :-1
            ]
        else:
        ...
        result = {
            ...
            "masks": {"loss_mask": loss_mask_slice, "input_alive": alive},
            "extras": {
        ...
        if "reset_mask" in batch_data:
            result["inputs"]["reset_mask"] = batch_data["reset_mask"][:, :-1].to(
