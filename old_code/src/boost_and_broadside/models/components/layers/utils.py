...
class RMSNorm(nn.Module):
    ...
    def forward(self, x):
        ...
        if hasattr(F, "rms_norm"):
            out = F.rms_norm(
                x_flat, (orig_shape[-1],), self.weight.to(x.dtype), self.eps
            )
        else:
            # Manual fallback
...
class MambaBlock(nn.Module):
    def __init__(
        self, d_model: int, d_state: int = 128, expand: int = 2, layer_idx: int = 0
    ):
        super().__init__()
        if Mamba2 is None:
            ...
            try:
                self.mamba = Mamba2(
                    d_model=d_model, d_state=d_state, expand=expand, layer_idx=layer_idx
                )
            except:
                self.mamba = nn.Identity()
