...
class StateEncoder(nn.Module):
    def __init__(
        self, input_dim: int, d_model: int, normalizer: FeatureNormalizer | None = None
    ):
        super().__init__()
...
class FlattenedActionEncoder(nn.Module):
    ...
    def forward(self, action):
        ...
        if action.shape[-1] == 3:
            ...
            flat_idx = (
                action[..., 0] * (NUM_TURN_ACTIONS * NUM_SHOOT_ACTIONS)
...
class RelationalEncoder(nn.Module):
    ...
    def compute_analytic_features(
        self, pos, vel, att=None, world_size=(1024.0, 1024.0)
    ):
        """
        ...
        if self.normalizer:
            ...
            dist_norm = self.normalizer.normalize(
                dist_transformed, "Relational_log_dist"
            )
