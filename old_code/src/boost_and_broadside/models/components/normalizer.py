...
from boost_and_broadside.core.features import (
    EGO_STATE_FIELDS,
    TARGET_FIELDS,
    RELATIONAL_FIELDS,
)
...
class FeatureNormalizer(nn.Module):
            ...
            for stat in ["Mean", "Std", "RMS", "Min", "Max"]:
                if stat in row:
                    ...
                    self.register_buffer(
                        name, torch.tensor(float(row[stat]), dtype=torch.float32)
                    )
    ...
    def _setup_group(self, group_name: str, fields: list[tuple[str, str]]):
        ...
        for feature, strategy in fields:
            ...
            try:
                ...
                log.warning(
                    f"Stat for {feature} not found, using identity for {group_name} group."
                )
            ...
            self.register_buffer(
                f"{feat_id}_shift_proc", torch.tensor(shift, dtype=torch.float32)
            )
            self.register_buffer(
                f"{feat_id}_scale_proc", torch.tensor(scale, dtype=torch.float32)
            )

        self.register_buffer(
            f"{group_name}_shift", torch.tensor(shifts, dtype=torch.float32)
        )
        self.register_buffer(
            f"{group_name}_scale", torch.tensor(scales, dtype=torch.float32)
        )
    ...
    def get_stat(self, feature: str, stat: str) -> torch.Tensor:
        ...
        raise AttributeError(
            f"Stat '{stat}' for feature '{feature}' not found in normalizer."
        )
