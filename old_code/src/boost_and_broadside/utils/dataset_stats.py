...
def calculate_action_counts(data_path: str) -> dict[str, np.ndarray]:
    ...
    try:
        ...
        mode = "r"
        log.warning(
            f"Could not open {data_path} in write mode. Caching to HDF5 attributes will be disabled."
        )

    with f:
        # Check attributes first
        if (
            "action_counts_power" in f.attrs
            and "action_counts_turn" in f.attrs
            and "action_counts_shoot" in f.attrs
        ):
        ...
        return {"power": counts_p, "turn": counts_t, "shoot": counts_s}
...
def calculate_flattened_action_counts(data_path: str) -> np.ndarray:
    ...
    try:
        ...
        try:
            ...
            log.warning(
                f"Could not open {data_path}. Returning uniform flattened weights."
            )
    ...
    with f:
        ...
        # p * 14 + t * 2 + s
        indices = (
            p_idx * (NUM_TURN_ACTIONS * NUM_SHOOT_ACTIONS)
...
def compute_class_weights(
    counts: np.ndarray, cap: float = 10.0, power: float = 0.5
) -> torch.Tensor:
    """
    ...
    # Standard frequency
    freq = counts.astype(np.float32) / total
    ...
    # Handle zero frequency by adding small eps before power or clipping after
    # x^(-power) will be inf where x is 0
    weights = torch.pow(x, -power)

    # Clip to maximum weight (this also handles the inf from 1/0)
...
def apply_turn_exceptions(weights: torch.Tensor) -> torch.Tensor:
    ...
    if len(weights) != NUM_TURN_ACTIONS:
        log.warning(
            f"Turn weights length {len(weights)} != {NUM_TURN_ACTIONS}. Skipping exceptions."
        )
        return weights
