...
class DamageReward(RewardComponent):
    ...
    def compute(
        ...
        damage_taken = torch.maximum(
            delta_health, torch.tensor(0.0, device=prev_state.device)
        )
...
class LookingAtEnemyReward(RewardComponent):
    ...
    def __init__(
        self,
        name: str = "looking_at_enemy",
        weight: float = 1.0,
        world_size: tuple = (1024.0, 1024.0),
    ):
        super().__init__(name, weight)
...
class RewardRegistry:
                ...
                if name == "damage":
                    self.components.append(
                        DamageReward(
                            ...
                            pain_weight=cfg.get("pain_weight", 1.0),
                            blood_weight=cfg.get("blood_weight", 1.0),
                        )
                    )
                elif name == "death":
                    self.components.append(
                        DeathReward(
                            weight=cfg.get("weight", 1.0),
                            die_penalty=cfg.get("die_penalty", 5.0),
                            kill_reward=cfg.get("kill_reward", 5.0),
                        )
                    )
                elif name == "victory":
                    self.components.append(
                        VictoryReward(
                            weight=cfg.get("weight", 1.0),
                            win_reward=cfg.get("win_reward", 100.0),
                            lose_penalty=cfg.get("lose_penalty", 100.0),
                        )
                    )
                elif name == "looking_at_enemy":
                    self.components.append(
                        LookingAtEnemyReward(weight=cfg.get("weight", 1.0))
                    )
    ...
    def compute_all(
        ...
        for comp in self.components:
            results[comp.name] = comp.compute(
                prev_state, actions, next_state, dones, is_terminal
            )
