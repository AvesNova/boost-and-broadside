...
- [raw live match history](../checkpoints/resilient-resonance-682/elo_history.jsonl);
...
`checkpoints/resilient-resonance-682/artifacts/`, and
[`docs/results/provenance.md`](results/provenance.md) — generated by `bnb publish` — maps
each output to the exact artifact behind it. Each artifact records its command, seeds,
resolved settings, checkpoint hashes, and the clean commit it was produced from, so the
numbers quoted below are traceable without reading this page.

The measurements were recomputed from the migrated checkpoint set with current code. They
are close to, but not identical with, the run's original evaluation: the environment
changed between the training commit and now, so ratings shift by a few tens of Elo and
crossover boundaries by at most two ships. Where a number below differs from an older
write-up, this page is the current one.
...
for instance, rests on 173 games at 88 scripted ships — a 50.3% rate with a binomial
standard error near 3.8 points, so read it as "roughly parity around 88", not as a
threshold located to the ship.
...
Key values from the calibration artifact named in
[the provenance index](results/provenance.md):

| Measurement | Elo | Conditional SE | Games |
|---|---:|---:|---:|
| Final checkpoint, 999.424M steps | 1772.2 | 7.3 | 9,347 tournament games |
| Final live policy (refit), 999.424M steps | 1771.7 | 18.3 | 627 recorded games |
| Last frozen ladder checkpoint, 876.495M steps | 1765.5 | 7.2 | 9,486 tournament games |
| Scripted controller | 1000 (anchor) | — | 26,633 tournament games |
| Random baseline | −335.3 | 10.5 | 8,391 tournament games |

The scripted controller is both the anchor and the fit's reference player here, so it
carries no conditional error of its own and every other rating's SE is already relative to
it. The final checkpoint's lead over scripted is about 772 points. The independent
[fleet-scale tournament](#symmetric-fleet-scale-ratings) below rates the same checkpoint
1787 ± 11 in its own 4-vs-4 bracket — the two measurements agree within uncertainty.
...
conditioned tournament places it at −335 ± 10, a reminder that saturated matchups carry
...
team roles and stores directed win/loss/tie counts in the elo-scale artifact.
...
*The 4-vs-4 checkpoint strengthens as the fleet grows zero-shot, peaking at 32-vs-32 in
...
|---:|---:|---:|---|
| 1 | 49,152 | 1492 ± 9 | yes |
| 2 | 49,152 | 1629 ± 10 | yes |
| 4 | 49,152 | 1787 ± 11 | yes |
| 8 | 78,125 | 1942 ± 11 | yes |
| 16 | 46,872 | 2128 ± 17 | no |
| 32 | 11,712 | 2220 ± 37 | no |
| 64 | 2,928 | 1958 ± 59 | no |

At the native 4-vs-4 scale this tournament rates the final checkpoint 1787 ± 11,
consistent with the calibration tournament's independent 1772.2 ± 7.3 above.

Each size gets the same budget — at most twelve adaptive batches — and each batch is
...
target and stop where they are rather than quoting a precision they did not buy; their
error bars are the honest reading. The peak's exact location is inside that uncertainty:
16, 32, and 64 do not separate cleanly from one another.
...
*Across seven fleet sizes, the stronger rung scores between 53.1% and 84.4% against its
neighbor over 70 adjacent comparisons. All but four fall in the shaded 20–80% target
region, and those four reach only 84.4%; none is close to a deterministic matchup.*

The saved reference tournament contains 128 side-balanced games for every unordered pair at
every scale. Its outcome matrices are joined to the checkpoint tournament through the
shared random and scripted endpoints before fitting, which improves graph connectivity
without treating an interpolated controller as a trained checkpoint.

The same measurement doubles as the check on training's live gauge, which assigns each
rung 1000·p instead of fitting it. Each scale in the `semi-random` artifact reports
`live_gauge_error`: the fitted ladder regauged to the same two endpoints, minus the rating
training uses. At 4-vs-4 the linear placement runs about 110 Elo high at the `p=0.2` rung
and within 17 points from `p=0.6` up — the approximation the
[live Elo contract](training.md#live-elo-versus-calibrated-elo) was accepted on, measured
again here.
...
Every entry names its source, and
[`docs/results/provenance.md`](results/provenance.md) is the generated map from each
canonical output back to it. A full calibration is launched with
...
captured as scratch under `out/`, which is never tracked; the manifest pins it by sha256
and publication copies it in. On a checkout where the scratch clip is gone, `--check`
verifies the tracked GIF against that committed hash instead, so a published clip is
always provably the file that was reviewed.
...
- The published measurements come from the migrated checkpoints under current physics, not
  from the environment the run was trained in; they are close to the original evaluation
