...
- [crossover sweep](../checkpoints/good-leaf-719/artifacts/figures/crossover.json);
- [curated seeded replays](results/replays/).

Every figure on this page *is* a file under
[`checkpoints/good-leaf-719/artifacts/`](../checkpoints/good-leaf-719/artifacts/), linked
where it lies rather than copied here. Its
[`figures` artifact](../checkpoints/good-leaf-719/artifacts/figures/artifact.json) records
which measurement each chart was drawn from and that measurement's digest, and each
measurement in turn records the command that produced it, its seeds, resolved settings,
checkpoint hashes, and the commit it was run from. Earlier runs are summarised in
...
![Policy-controlled vs scripted-controlled crossover](../checkpoints/good-leaf-719/artifacts/figures/crossover_phase.png)
...
![Scripted ships beaten per policy-controlled ship](../checkpoints/good-leaf-719/artifacts/figures/crossover_ratio.png)
...
![Calibrated live-policy Elo](../checkpoints/good-leaf-719/artifacts/figures/elo_curve.png)
...
![Fleet-scale Elo with scripted anchored at 1000](../checkpoints/good-leaf-719/artifacts/figures/elo_scale_scripted_1000.png)
...
![Reference-ladder connectivity](../checkpoints/good-leaf-719/artifacts/figures/semi_random_connectivity.png)
...
![Win rate against the scripted controller](../checkpoints/good-leaf-719/artifacts/figures/win_rate_vs_scripted.png)
...
the [autoregressive reports](../checkpoints/good-leaf-719/artifacts/figures/ar_report_4v4/) and [noise calibration](../checkpoints/good-leaf-719/artifacts/figures/noise_calibration/).

## Training health

![Training diagnostics](../checkpoints/good-leaf-719/artifacts/figures/training_health.png)
...
Every figure comes from one command, run against a finished run's own measurements:

```bash
uv run bnb figures --run good-leaf-719
uv run bnb figures --run good-leaf-719 --only elo_curve.png
```

It writes into `checkpoints/<run>/artifacts/figures`, which is where these pages link. So
there is exactly one copy of each chart, and illustrating the documents with a different
run is a matter of pointing the links at it rather than of regenerating anything. Charts
are checked against the schema of the measurement they read, so an artifact written before
a schema change is refused by name instead of drawn from fields that no longer mean what
the renderer expects.

Rendering is offline and runs no simulation, so it needs no GPU. Refitting a stored
calibration also needs none: `uv run bnb elo-calibrate --from-artifact <artifact
directory>` refits the saved win/tie matrices and writes a new artifact. Rerunning the
measurements themselves does need one; see
[getting started](getting-started.md#evaluate).

The curated replays are the one class of output no renderer produces. A clip is captured
as scratch under `out/`, reviewed, then copied into `docs/results/replays/` and committed.
The committed file is the clip.
