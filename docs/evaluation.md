...
[`checkpoints/resilient-resonance-682/artifacts/`](../checkpoints/resilient-resonance-682/artifacts/).
...
each figure to the exact artifact behind it.

The run targeted one billion environment steps and logged 999,424,000 of them in 7.50
hours on a single RTX 5090, at a final throughput of about 296,000 ship-tokens/s (37,000
environment steps/s at 8 ships per environment). Both figures come from the
[W&B summary](../checkpoints/resilient-resonance-682/wandb_export/summary.json) and
[run metadata](../checkpoints/resilient-resonance-682/wandb_export/files/wandb-metadata.json).
Ship-tokens/s is the primary throughput measure here because it stays comparable when
ships-per-environment changes; steps/s does not.
...
Key values from the
[calibration artifact](../checkpoints/resilient-resonance-682/artifacts/elo-calibration/):
...
| Scripted controller | 1000 (anchor) | n/a | 26,633 tournament games |
...
1787 ± 11 in its own 4-vs-4 bracket, agreeing within uncertainty.

The rungs matter more than they look. Before they were added, random's position was
measured only through near-certain games against far stronger players and read about
−240 ± 33; the conditioned tournament places it at −335 ± 10. Saturated matchups carry
...
[fleet-scale artifact](../checkpoints/resilient-resonance-682/artifacts/elo-scale/).
...
target and stop where they are, so their error bars are wide. The peak's exact location
sits inside that uncertainty: 16, 32, and 64 do not separate cleanly from one another.
...
The saved
[reference tournament](../checkpoints/resilient-resonance-682/artifacts/semi-random-ladder/)
