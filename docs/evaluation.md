...
of fitting it. Each scale reports a `live_gauge_error`: the fitted ladder regauged to the
same two endpoints, minus the rating training uses. At 4-vs-4 the linear placement runs
about 110 Elo high at the `p=0.2` rung and within 17 points from `p=0.6` up. The
[live Elo contract](training.md#live-elo-versus-calibrated-elo) was accepted on that
approximation.
