...
class BaseView(Dataset):
    def __init__(self, dataset: UnifiedEpisodeDataset, indices: list[int], seq_len: int):
...
class ShortView(BaseView):
    def __init__(self, dataset: UnifiedEpisodeDataset, indices: list[int], seq_len: int = 32):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        seq_actions = self._get_shifted_actions_from_full(abs_start, abs_end, start_offset)
        ...
        if actual_len < self.seq_len:
            ...
            token_pad = torch.zeros(pad_len, *seq_tokens.shape[1:], dtype=seq_tokens.dtype, device=seq_tokens.device)
            seq_tokens = torch.cat([seq_tokens, token_pad], dim=0)
            
            action_pad = torch.zeros(pad_len, *seq_actions.shape[1:], dtype=seq_actions.dtype, device=seq_actions.device)
