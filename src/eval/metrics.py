    ...
    with torch.no_grad():
        for i, (states, actions, returns, mask, action_masks) in enumerate(val_loader):
            ...
            gen_states, _ = model.generate(
                initial_state, initial_action, steps=steps_to_gen, n_ships=states.shape[2]
            )
...
def compute_controlling_error(model, env_config, device, max_episode_length=200, num_episodes=1):
    """
    ...
    for _ in range(num_episodes):
        ...
        while not done and steps < max_episode_length:
            ...
            mask = torch.zeros(1, seq_len, agent.max_ships, dtype=torch.bool, device=device)
            mask[:, -1, :] = True
