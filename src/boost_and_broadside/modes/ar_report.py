...
def run_ar_report_mode(
    ...
    agent0 = resolve_agent_spec(team0_spec, ship_config, model_config, device, checkpoint_dir, num_ships=env_config.num_ships)
    agent1 = resolve_agent_spec(team1_spec, ship_config, model_config, device, checkpoint_dir, num_ships=env_config.num_ships)
    ...
    history_closed = _run_ar(
        agent0, agent1, init_obs, init_hidden0, init_hidden1, actual_steps, N, ship_config, actions_sim, True
    )

    print("Running AR Rollout (Open Loop)...")
    history_open = _run_ar(
        agent0, agent1, init_obs, init_hidden0, init_hidden1, actual_steps, N, ship_config, None, False
    )
...
def _run_ar(
    ...
    for step in range(num_steps):
        ...
        action1, pred_next1 = get_actions(agent1, obs, None, 1, N, obs["pos"].device, return_pred_next=True)
        ...
        action_to_apply = forced_actions[step] if is_closed_loop and forced_actions is not None else imag_action
...
def _generate_report(history_sim, history_closed, history_open, ship_config, num_steps, out_dir):
    ...
    def plot_trajectory_on_ax(ax_target, pos_arr_all, alive_arr_all, label, method_color, W_x, W_y):
        ...
        for s in range(n_ships):
            ...
            if len(dead_idx) > 0:
                d_idx = dead_idx[0]
                ax_target.plot(pos_arr[d_idx, 0], pos_arr[d_idx, 1], marker='x', color='red', markersize=5, mew=1.5, zorder=5)

            # Draw dots first (lower z-order)
            for i in range(len(pos_arr)):
                if i % 10 == 0:
                    ax_target.plot(pos_arr[i, 0], pos_arr[i, 1], marker='o', color=dot_color, markersize=2, zorder=2)
            ...
            for i in range(len(pos_arr) - 1):
                ...
                if dist_x < W_x / 2 and dist_y < W_y / 2:
                    ax_target.plot(
                        ...
                        color=method_color, alpha=0.6, lw=1.0, zorder=3
                    )
    ...
    if num_ships > 2:  # 2v2+ only
        ...
        plot_trajectory_on_ax(ax_map, sim_pos_all, sim_alive_all, "Ground Truth", METHOD_COLORS['gt'], W_x, W_y)
        plot_trajectory_on_ax(ax_map, cl_pos_all, cl_alive_all, "AR Closed Loop", METHOD_COLORS['cl'], W_x, W_y)
        plot_trajectory_on_ax(ax_map, ol_pos_all, ol_alive_all, "AR Open Loop", METHOD_COLORS['ol'], W_x, W_y)
        ax_map.legend()
    ...
    def plot_centered_trajectory(ax_target, pos_c, alive_arr_all, label, method_color):
        for s in range(plot_N):
            ...
            if len(dead_idx) > 0:
                d_idx = dead_idx[0]
                ax_target.plot(pos_s[d_idx, 0], pos_s[d_idx, 1], marker='x', color='red', markersize=5, mew=1.5, zorder=5)

            # Draw dots first (lower z-order)
            for i in range(len(pos_s)):
                if i % 10 == 0:
                    ax_target.plot(pos_s[i, 0], pos_s[i, 1], marker='o', color=dot_color, markersize=2, zorder=2)
    ...
    plot_centered_trajectory(ax_map2, sim_pos_c, alive_for_plot, "Ground Truth", METHOD_COLORS['gt'])
    plot_centered_trajectory(ax_map2, cl_pos_c, cl_alive_for_plot, "AR Closed Loop", METHOD_COLORS['cl'])
    plot_centered_trajectory(ax_map2, ol_pos_c, ol_alive_for_plot, "AR Open Loop", METHOD_COLORS['ol'])
    ax_map2.legend()
    ...
    def plot_vel_trajectory(ax_target, vel_arr, alive_arr_all, label, method_color):
        for s in range(plot_N):
            ...
            if len(dead_idx) > 0:
                d_idx = dead_idx[0]
                ax_target.plot(vel_s[d_idx, 0], vel_s[d_idx, 1], marker='x', color='red', markersize=5, mew=1.5, zorder=5)

            # Dots first
            for i in range(len(vel_s)):
                if i % 10 == 0:
                    ax_target.plot(vel_s[i, 0], vel_s[i, 1], marker='o', color=dot_color, markersize=2, zorder=2)
    ...
    plot_vel_trajectory(ax_vel_map, cl_vel, cl_alive_for_plot, "AR Closed Loop", METHOD_COLORS['cl'])
    ...
    mae_features = [
        ...
        ("attitude", "Attitude Error (MAE)", calc_mae(cl_att, sim_att, cl_alive, sim_alive), calc_mae(ol_att, sim_att, ol_alive, sim_alive)),
        ("health", "Health Error (MAE)", calc_mae(cl_health, sim_health, cl_alive, sim_alive), calc_mae(ol_health, sim_health, ol_alive, sim_alive)),
        ("power", "Power Error (MAE)", calc_mae(cl_power, sim_power, cl_alive, sim_alive), calc_mae(ol_power, sim_power, ol_alive, sim_alive)),
    ]
    ...
    for (file_key, name, err_cl, err_ol) in mae_features:
        ...
        for s in range(plot_N):
            ...
            ax_err.plot(steps, err_cl[:, s], label=f'CL (S{s})', color=METHOD_COLORS['cl'], linestyle=l_style)
            ax_err.plot(steps, err_ol[:, s], label=f'OL (S{s})', color=METHOD_COLORS['ol'], linestyle=l_style, alpha=0.8)
    ...
    features = [
        ...
        ("angular_vel_scaled", "Angular Vel (Scaled to GT)", sim_ang_vel[..., 0], cl_ang_vel[..., 0], ol_ang_vel[..., 0]),
    ...
    for (file_key, name, sim_f, cl_f, ol_f) in features:
        ...
        for s in range(plot_N):
            l_style = '--' if s >= num_ships // 2 else '-'
            if name == "Alive Prob":
                ax.plot(steps, sim_f[:, s].astype(float), label=f'GT (S{s} Alive)', color=METHOD_COLORS['gt'], linestyle=l_style)
            else:
                ax.plot(steps, sim_f[:, s], label=f'GT S{s}', color=METHOD_COLORS['gt'], linestyle=l_style)
            ax.plot(steps, cl_f[:, s], label=f'CL S{s}', color=METHOD_COLORS['cl'], alpha=0.9, linestyle=l_style)
            ax.plot(steps, ol_f[:, s], label=f'OL S{s}', color=METHOD_COLORS['ol'], alpha=0.9, linestyle=l_style)
    ...
    with open(os.path.join(out_dir, "ar_report.md"), "w") as f:
        ...
        f.write("This report compares the ground truth simulation with closed-loop (forced actions) and open-loop (imagined actions) autoregressive rollouts.\n\n")
