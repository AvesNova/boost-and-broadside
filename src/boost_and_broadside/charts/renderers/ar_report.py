"""The autoregressive rollout report: a rendering contract over a stored episode.

Filenames, headings, metric definitions, ship ordering, death markers, the
every-10-steps trajectory dot sampling, and toroidal-wrap handling are all part
of what a reader is promised, so they live here in one place and are rendered
from the measurement rather than alongside it. Nothing here replays a game.
"""
...
# Suppress the library version stamp so a re-render of the same measurement is
# byte-identical, so re-rendering a run's figures does not churn its artifact.
...
def _extract_rollout_arrays(
    arrays: dict[str, np.ndarray], prefix: str, plot_N: int, W_x: float, W_y: float
) -> _RolloutArrays:
    """Pull every plotted feature for one rollout out of the stored arrays."""

    def field(key: str) -> np.ndarray:
        return arrays[f"{prefix}_{key}"]

    def featured(key: str) -> np.ndarray:
        return field(key)[:, :plot_N].astype(float)

    pos = featured("pos")
    alive_all = field("alive") > 0.5
...
def _plot_full_world_map(
    ...
    fig_map.savefig(os.path.join(out_dir, "2d_map.png"), metadata=_DETERMINISTIC)
    plt.close(fig_map)
...
def _plot_centered_map(
    ...
    fig_map2.savefig(os.path.join(out_dir, map2_filename), metadata=_DETERMINISTIC)
    plt.close(fig_map2)
...
def _plot_velocity_map(
    ...
    fig_vel_map.savefig(os.path.join(out_dir, "2d_vel_map.png"), metadata=_DETERMINISTIC)
...
def _plot_error_metrics(
    ...
    for file_key, name, err_cl, err_ol in mae_features:
        ...
        fig_err.savefig(os.path.join(out_dir, f"mae_{file_key}.png"), metadata=_DETERMINISTIC)
        plt.close(fig_err)
...
def _plot_feature_divergence(
    ...
    for file_key, name, sim_f, cl_f, ol_f in features:
        ...
        fig_feat.savefig(os.path.join(out_dir, f"feature_{file_key}.png"), metadata=_DETERMINISTIC)
        plt.close(fig_feat)
...
def generate_report(result: dict, arrays: dict[str, np.ndarray], out_dir: str) -> None:
    """Render the complete AR report from one stored measurement."""

    os.makedirs(out_dir, exist_ok=True)

    num_ships = int(result["num_ships"])
    num_steps = int(result["num_steps"])
    plot_N = 2 if num_ships == 2 else 1
    W_x, W_y = (float(value) for value in result["world_size"])

    gt = _extract_rollout_arrays(arrays, "gt", plot_N, W_x, W_y)
    cl = _extract_rollout_arrays(arrays, "cl", plot_N, W_x, W_y)
    ol = _extract_rollout_arrays(arrays, "ol", plot_N, W_x, W_y)
...
def _render(inputs: RenderInputs, out_dir: Path) -> list[Path]:
    artifact = inputs.artifact("ar_report")
    result = artifact.read_json()
    with np.load(artifact.path / "result.npz") as stored:
        arrays = {key: stored[key] for key in stored.files}
    generate_report(result, arrays, str(out_dir))
...
register(
    Renderer(
        name="ar-report-v1",
        description="Closed- and open-loop imagined rollouts against ground truth.",
        render=_render,
        required_artifacts=("ar_report",),
        supported_schemas={"ar_report": (1,)},
        multi_file=True,
    )
)
