...
Implements standard multi-head self-attention with an alive-ship mask.
The "relational" aspect comes from all ships attending to all other ships —
the per-ship position tokens carry the spatial information.
...
class RelationalSelfAttention(nn.Module):
    """Multi-head self-attention over N ship tokens with alive masking.
