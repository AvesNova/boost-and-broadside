...
class BoostAndBroadsideProblem(Problem):
    def __init__(self, cfg: DictConfig, base_env: TensorEnv):
        ...
        default_v = torch.tensor(StochasticAgentConfig.default_vector(), device=self.device, dtype=torch.float32)
    ...
    def _evaluate_batch(self, solutions):
        ...
        config_tensor = torch.zeros(total_envs, eval_env.max_ships, 24, device=self.device)
...
class SPSLogger(StdOutLogger):
    ...
    def _log(self, status: dict):
        ...
        sps = (self.popsize * self.max_episode_steps * self.matches_per_eval) / (elapsed + 1e-8)
        ...
        print(f"Gen {step:04d} | Max Fit: {best_eval:.3f} | Mean Fit: {mean_eval:.3f} | Median Fit: {median_eval:.3f} | Max Theoretical SPS: {int(sps)}")
...
class EvoTrainer:
    ...
    def train(self):
        ...
        if problem.shared_parameters:
            searcher.population.access_values(keep_evals=True)[0] = problem.baseline_agent[0].reshape(-1)
        else:
            searcher.population.access_values(keep_evals=True)[0] = problem.baseline_agent.reshape(-1)
        
        _ = SPSLogger(searcher, self.popsize, problem.max_episode_steps, self.matches_per_eval)
