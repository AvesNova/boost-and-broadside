...
from boost_and_broadside.agents.team_transformer_agent import TeamTransformerModel
from boost_and_broadside.train.data_loader import load_bc_data, create_bc_data_loader
...
def train_bc(cfg: DictConfig) -> Path | None:
    ...
    for epoch in range(epochs):
        ...
        for batch_idx, (tokens, actions, _, returns, *_) in enumerate(train_pbar):
            ...
            returns = returns.to(device).squeeze(1)  # (batch,)
        ...
        with torch.no_grad():
            ...
            for batch_idx, (tokens, actions, _, returns, *_) in enumerate(val_pbar):
                ...
                returns = returns.to(device).squeeze(1)
