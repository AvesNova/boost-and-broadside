...
class WorldModelSB3Policy(ActorCriticPolicy):
    ...
    def forward(
        ...
        pred_states, pred_action_logits, _, _, _, embeddings = self.world_model(
            states, actions, return_embeddings=True
        )
    ...
    def evaluate_actions(
        ...
        # Run model
        pred_states, pred_action_logits, _, _, _, embeddings = self.world_model(
            states, prev_actions, return_embeddings=True
        )
    ...
    def predict_values(self, obs: torch.Tensor) -> torch.Tensor:
        states, prev_actions = self._get_obs_actions(obs)
        _, _, _, _, _, embeddings = self.world_model(
            states, prev_actions, return_embeddings=True
        )
    ...
    def get_dynamics_loss(self, obs: torch.Tensor) -> torch.Tensor:
        ...
        from boost_and_broadside.utils.tensor_utils import (
            to_one_hot,
        )  # Import locally to avoid circular dep if needed

        input_actions_oh = to_one_hot(input_prev_actions)

        # Run Forward
        pred_states, pred_actions, pred_value, _, _ = self.world_model(
            input_states, input_actions_oh, mask_ratio=0.0, noise_scale=0.0, mask=None
        )
